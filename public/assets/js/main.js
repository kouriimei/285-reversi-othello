...
socket.on('player_disconnected', (payload) =>{
	...
	let domElements = $('.socket_' + payload.socket_id);
	if(domElements.length !== 0) {domElements.hide("fade", 500);}
...
/********************enforcing turns***************/
	...
	if(my_color === "Light") {
		$("#my_color").html('<h3 id = "my_color">I am Light</h3>');
	}

	else if(my_color === "Dark") {
		$("#my_color").html('<h3 id = "my_color">I am Dark</h3>');
	}
	...
	if(payload.game.whose_turn === "Light") {
		$("#my_color").append('<h4>It is Light\'s turn</h4>');
	}

	else if(payload.game.whose_turn === "Dark") {
		$("#my_color").append('<h4>It is Dark\'s turn</h4>');
	}

	else {
		$("#my_color").append('<h4>Error: Don\'t know who\'s turn it is</h4>');
	}
	...
	for (let row = 0; row < 8; row++) {
		for (let column = 0; column < 8; column++) {
			if (board[row][column] === "L") {whitesum++;}
			else if (board[row][column] === "D") {blacksum++;}
	...
	$("#lightsum").html(whitesum);
	$("#darksum").html(blacksum);
	old_board = board;
})
